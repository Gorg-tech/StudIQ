...
IMPORTANT: Django erstellt **keine** Datenbank automatisch. MySQL/MariaDB muss manuell installiert und die Datenbank samt Benutzer erstellt werden.
...
NOTE: Weitere Informationen zur MySQL-Installation finden Sie in der https://dev.mysql.com/doc/refman/8.0/en/installing.html[offiziellen MySQL-Dokumentation].
...
CREATE USER 'studiq_user'@'localhost' IDENTIFIED BY 'IHR_SICHERES_PASSWORT';
GRANT ALL PRIVILEGES ON studiq_db.* TO 'studiq_user'@'localhost';
FLUSH PRIVILEGES;
EXIT;
----

IMPORTANT: Ersetzen Sie `IHR_SICHERES_PASSWORT` durch ein sicheres Passwort und verwenden Sie dasselbe Passwort in der `.env`-Datei!
...
==== Webserver-Konfiguration (Produktion)
...
server {
    listen 80;
    server_name ihre-domain.de;  # Beispiel für StudIQ-Produktionssystem: studiq.oceanbound.io
...
NOTE: Weitere Informationen zur Nginx-Konfiguration finden Sie in der https://nginx.org/en/docs/[offiziellen Nginx-Dokumentation].
...
Weitere Informationen: https://certbot.eff.org/[Certbot-Dokumentation]
...
* https://nginx.org/en/docs/[Nginx Dokumentation]
* https://docs.docker.com/[Docker Dokumentation]
* https://letsencrypt.org/de/docs/[Let's Encrypt Dokumentation]
