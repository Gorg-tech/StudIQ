...
Leider wurde bei uns zu Beginn zu viel Wert darauf gelegt, einen vorzeigbaren Prototypen mit den entsprechenden Features zu entwickeln, wodurch mehr Zeit in neu entwickelten Code investiert wurde, anstatt diese in eine verständliche Dokumentation des bisher entwickelten Codes zu stecken.
...
So hat dort eine Person das Grund-Framework errichtet und dann bereits einige neue Funktionen programmiert, welche noch benötigt wurden. 
Diese Person hat zwar die wichtisten Funktionen kommentiert, jedoch gab es immer noch einen hohen Einarbeitungsaufwand bei neu hinzukommenden Backend-Entwicklern.
...
Zum anderen hatten wir keine Dokumentationsstandards, wie zum Beispiel Docstrings an jeder Funktion, da wir uns der Existenz dieser damals noch nicht bewusst waren, und, weil wir, wie bereits erwähnt, zu Beginn relativ schnell unseren Prototyp fertig stellen wollten.
...
Diesem Problem hätte man vorbeugen können, wenn wir uns bereits früh auf Dokumentier-Standards geeinigt hätten, was ebenfalls später in der Erstellung der Betriebsdokumentation sehr hilfreich gewesen wäre.
...
Diese Dokumentier-Standards zum Einen damals bereits ein bisschen mehr beim Verstehen des Code helfen können.
...
Dafür spricht, dass die nun damit erstellte Betriebsdoku laut einigen unserer Entwickler die Backend-Struktur sehr gut verdeutlicht und zu Beginn der Software-Entwicklung sehr hilfreich gewesen wäre, um den Code schnell zu verstehen.
So hätten auch im späteren Prozess neue Backend-Entwickler keine große Hilfe von Entwicklern des bestehenden Codes benötigt. 

So kann dann neben der investierten Zeit der neuen Entwickler in das Verstehen des Code auch die investierte Zeit der bestehenden Entwickler in das Erklären des Codes verringert werden, auch wenn diese etwas mehr Zeit in die Dokumentation investieren müssen. 
...
Ebenfalls wurde eine Person festgelegt, welche sich mit einem Dokumentationstool wie Doxygen beschäftigen sollte, um damit den entwicklten Code verständlich, auch mit Diagrammen, erklären zu können.
...
Da wir dennoch stetig Fortschritte in der Entwicklung von Features gemacht haben, schien uns die ineffiziente Bewertung von User Stories neben der langen Meeting-Zeit ebenfalls nicht als wichtiges Problem aufzufallen.
